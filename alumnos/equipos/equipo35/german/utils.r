...
load<- function()
{
  if (file.exists("german.rds")){
    ...
    german.url <- paste('http://archive.ics.uci.edu/ml/machine-learning-databases/statlog/german/german.data',sep='')
    german.data <- read.table(german.url,stringsAsFactors = FALSE, header = FALSE)
...
german.decode <- function(columna)
{
    ...
    if(class(columna)=='character'){
        columna <- as.character(german.codes[ columna ])
...
arreglacols <- function(german.data)
  {
  ...
  german.data$Good.Loan <- as.factor(ifelse(german.data$Good.Loan == 1, 'GoodLoan', 'BadLoan'))
  german.colnames <- str_replace_all(german.colnames, ' ', '.')
  german.colnames <- str_replace_all(german.colnames, '/', '.')

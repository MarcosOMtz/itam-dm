...
# ponemos los nombres a las variables 
colnames(german.data) <- german.colnames

# arreglamos la variable good loan
...
mapeo <- function(columna){
  if(class(columna)=='character'){
    Reduce(  rbind, german.codes[ columna ])
    #recode(var = german.data[,1], recodes = german.codes)
  }else{
    columna
  }
}

german.data <- german.data %.% mutate_each(  funs( mapeo) ) 





#tarea  cambiar espacios por puntos y mayusculas por minusculas 
#stringr  o grep ggvis lubridate  lapply  leer branches (git)  explorar git 
